### aggregateProfileBased is a function that performs aggregation according to
### the profile based approach in Speelman et al 2003

aggregateProfileBased <- function(initialization, dstat="llr", cweight="w"){
	...
	while (i <= length(initialization$raw.frequencies)){
		for (var1 in initialization$lect.names){
			for (var2 in initialization$lect.names){
				cweight <- ( sum(as.vector(initialization$raw.frequencies[[i]][var1,])) + 
							 sum(as.vector(initialization$raw.frequencies[[i]][var2,])) ) /
						   ( initialization$lect.frequencies[var1] + 
						     initialization$lect.frequencies[var2] )
